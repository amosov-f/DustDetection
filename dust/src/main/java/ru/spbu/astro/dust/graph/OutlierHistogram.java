...
public class OutlierHistogram {
    ...
    public <T extends Comparable<T>> OutlierHistogram(@NotNull final List<Star> stars, @NotNull final StarCounter<T> starCounter) {
        missStars = new StarSelector(stars).negativeExtinction().getStars();

        final DefaultCategoryDataset dataset = new DefaultCategoryDataset();

        final Map<T, Integer> outlierCounts = starCounter.count(missStars);
        final int outlierCount = IntStream.of(Ints.toArray(outlierCounts.values())).sum();
        final Map<T, Integer> counts = starCounter.count(stars);
        ...
        JFreeChart chart = ChartFactory.createBarChart(
                String.format(
                        ...
                        starCounter.getName(), 100 * outlierCount / count, outlierCount, count),
                starCounter.getName(),
                "Доля",
                dataset,
